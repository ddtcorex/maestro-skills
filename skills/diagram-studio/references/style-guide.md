...
- **Cluster header padding via mermaid init + spacer + label y-offset, not CSS padding** — use `%%{init: {'flowchart': {'padding': 24, 'nodeSpacing': 24, 'rankSpacing': 60}}}%%` for bottom gap; for top gap (header touching rect top: label y=8 vs rect y=8, 0px gap, still sát after init) add `padRuntime --> GovardCLI` spacer + post-process SVG: `class="cluster-label" transform="translate(x, y+16)"` (label y 8→24, 16px top gap). CSS `span {padding}` only enlarges label, not rect.

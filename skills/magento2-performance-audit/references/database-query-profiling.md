...
> Per-page counts feed `audit-data.json` (`references/report-template.md`): one entry per captured page (`label, url, uri, totalQueries, distinctShapes, shapes`), rendered into the Repeated Shapes and Per-Page Detail tables — never scrape the rendered markdown back.
...
> **two-pass strategy (Two-pass) keeps log volume sane.** `--include-call-stack=true` walks and serializes a full PHP stack trace for *every* query — several MB per page, and that compounds fast once you're capturing 3 page types (see `references/per-page-type-audit.md` quick/deep table: quick `call-stack false` threshold 1 vs deep `call-stack true` threshold 0, batch govard sh, `maestro_perf_log_stats` streaming, trap single). Run **pass 1** with `--include-call-stack=false` `--query-time-threshold=1` (quick) or `--query-time-threshold=0` (deep first scan) across every page you're auditing — repeated shapes/counts are all you need to spot an N+1 candidate; only re-enable **pass 2** with `--include-call-stack=true` for a second, targeted re-capture of the specific 1–2 page(s) whose repeated shape you're now tracing to a file:line. Stack-walking every page from the start is the slower, heavier default — reserve it for the one or two pages that actually need it. On DSH, use `maestro_perf_log_stats` streaming (bounded 2 MiB, server-side `cat var/debug/db.log`) instead of hand-grep of the raw log; otherwise `grep -c '## QUERY'` locally.
...
**3. For each slow query found, `EXPLAIN` it before guessing at a fix — and validate on production-sized data:**

```bash
govard db query "EXPLAIN <the exact slow SQL, with real values substituted for placeholders>"
# Also check table size — a 200k+ row table that EXPLAINs as type: ALL locally is a prod fire:
govard db query "SELECT COUNT(*) FROM <table>"
```

Read the `type` and `key` columns first — `type: ALL` (full table scan) or an empty `key` (no index used) on a query filtering/joining on a non-trivial row count is the smoking gun. Common Magento-specific causes: a `WHERE` on a custom/EAV attribute column with no matching index, a report/export query missing a composite index that matches its actual filter+sort combination, or a JOIN condition that doesn't line up with either side's index. Cross-reference against the `Missing Index`/`Expensive Join` rows in Common Query Issues above — this `EXPLAIN` step is how you confirm those heuristic grep hits are real, rather than reporting a `WHERE` clause as a finding on pattern-match alone. For any candidate that touches a large table (200k+ rows in production — check `catalog_product_entity`, `sales_order`, EAV value tables on prod snapshot size, not local dev size), run `EXPLAIN` with the prod row estimate in mind: even a fast local plan is suspect if `type: ALL` and `rows` would be 200k in prod. Note the prod row count in the report (`Skipped: EXPLAIN prod-sized 200k validation — local table only N rows`) if you cannot confirm.

> **pt-query-digest for richer analysis:** when available, `pt-query-digest <slow_query_log_file>` groups by fingerprint and ranks by total/average time, 95th percentile, and lock time — richer than `mysqldumpslow -s t -t 10` (top 10 by total time). Prefer `pt-query-digest` when the toolkit is installed; fall back to `mysqldumpslow` otherwise. Both read the same slow log set up via the Govard-owned DB lifecycle lock (no `SET GLOBAL slow_query_log=ON` hand-edit — report `Skipped: DB slow log needs Govard-owned DB lifecycle lock` if not available).

> **Local dev DBs are small and fast** — the absolute query *time* on a local box will often look fine (tens of milliseconds total) even when the query *count* is far over budget, and a query that would be a real `type: ALL` full-scan problem on production's actual row counts can run in a few ms locally with a table of 50 rows. Raw count is what matters for the N+1 check above; for this slow-query check, don't trust a fast local `EXPLAIN` on a near-empty table as proof the query is fine at production scale — check `type`/`key` on the query plan itself, not just how fast it happened to run against this box's data. Streaming via `maestro_perf_log_stats` (DSH) is bounded 2 MiB; batch `govard sh` + `trap single` keeps the lock life clean.

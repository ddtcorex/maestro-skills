...
|------|-------|----------------------|----------------------|-------------|---------------|----------|---------------------|
| quick | 3 pages small/medium/large (1 home + 1 category + 1 product) — call-stack false — threshold 1 — quick files — grid_per_page 48 — cap 5–10m | `--include-call-stack=false` | `--query-time-threshold=1` | quick files (`app/code` + `app/design` + `app/etc` sampled, batch govard sh) | 48 | 5–10m | On DSH: call `maestro_perf_log_stats` streaming (bounded 2 MiB, server-side `cat var/debug/db.log`) / Otherwise: grep; batch govard sh single setup cmd; trap single |
| deep | 7 pages double-pass (1 home + 3 category small/medium/large + 3 product) — call-stack true — threshold 0 — deep files — 20–30m | `--include-call-stack=true` (two-pass: pass 1 false for counts, pass 2 true for 1–2 traces) | `--query-time-threshold=0` | deep files (full `app/code` + `vendor`/`dev`/`lib`/`m2-hotfixes` to the ignore boundary) | prod `grid_per_page`/`list_per_page` | 20–30m | Same On DSH/Otherwise split; batch govard sh where possible; trap single (one `trap ... EXIT` for the whole session) |
